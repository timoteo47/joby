...
def ping(ip):
    completed_process = subprocess.run(["ping", "-c", "{}".format(PING_COUNT), "-q", "-o", ip], capture_output=True)
    return ip, completed_process.returncode


def ping_test(subnet_a, subnet_b, start_ip, end_ip, excluded_ips):
    futures = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        for i in range(start_ip, end_ip + 1):
            if i not in excluded_ips:
                futures.append(pool.submit(ping, "{}.{}".format(subnet_a, i)))
                futures.append(pool.submit(ping, "{}.{}".format(subnet_b, i)))
